...
class SourceItem:
    ...
    def __init__(self, landell, name, a_group, b_group):
        ...
        self.landell.connect("pipeline-ready", self._on_pipeline_ready)

    def source_destroy(self):
        self.landell.disconnect_by_func(self._on_pipeline_ready)

    def _on_pipeline_ready(self, landell):
        thumbnail = self.landell.get_thumbnail(self.name)
    ...
    def _on_b_press(self, event):
        if event.get_active():
            self.landell.set_pip_source(self.name)
            ...
            for action in self.b_group.list_actions():
                if action.get_active():
                    active = True
            if not active:
                self.landell.set_pip_source(None)
